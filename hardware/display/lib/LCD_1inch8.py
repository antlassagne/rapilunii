...
class LCD_1inch8(lcdconfig.RaspberryPi):
    ...
    def clear(self, color=0xFFFF):
        ...
        if (
            (self.LCD_Scan_Dir == L2R_U2D)
            or (self.LCD_Scan_Dir == L2R_D2U)
            or (self.LCD_Scan_Dir == R2L_U2D)
            or (self.LCD_Scan_Dir == R2L_D2U)
        ):
