...
class AllTalkController:
    def __init__(self, config_file="config.json"):
        logging.info("Hello AllTalkController!")
        ...
        self.base_url = f"{self.config['api_alltalk_protocol']}{self.config['api_alltalk_ip']}:{self.config['api_alltalk_port']}"
    ...
    def check_server_ready(self):
        ...
        Attempts to connect to the server within the specified timeout
        period. Returns True if the server is ready, False otherwise.
    ...
    def initialize(self):
        """
        Perform initial setup by fetching current settings and available
        voices.

        This method should be called after creating an instance of
        AllTalkAPI.
    ...
    def get_current_settings(self):
        ...
        Returns:
            dict: A dictionary of server settings or None if the request
        fails.
        """
        try:
    ...
    def get_available_voices(self):
        ...
        Returns:
            list: A list of available voices or None if the request fails.
        """
        try:
    ...
    def get_available_rvc_voices(self):
        ...
        RVC (Retrieval-based Voice Conversion) voices are used for voice
        cloning.

        Returns:
            list: A list of available RVC voices or None if the request fails.
        """
        try:
    ...
    def reload_config(self):
        ...
        This method triggers a config reload on the server and then re-
    ...
    def generate_tts(
        ...
        **kwargs,
        ...
        Args:
            ...
            character_voice: The voice to use for the character.
            output_file: The requested filepath of the generated voice
            narrator_voice: The voice to use for the narrator, if applicable.
        ...
        if result:
            ...
            if response.status_code == 200:
                ...
                logging.info("Failed to download TTS audio file.")
        else:
            return False
    ...
    def stop_generation(self):
        ...
        Returns:
            The server's response as a dictionary, or None if the
        request fails.
        """
    ...
    def set_deepspeed(self, enabled: bool):
        """
        Enable or disable DeepSpeed mode. DeepSpeed is an optimization library
        ...
        """
        response = requests.post(
            f"{self.base_url}/api/deepspeed",
            params={"new_deepspeed_value": str(enabled).lower()},
        )
        return response.json() if response.status_code == 200 else None

    def set_low_vram(self, enabled: bool):
        """
        Enable or disable Low VRAM mode. Low VRAM mode optimizes memory usage
        for systems with limited GPU memory.

        Args:
            enabled: True to enable Low VRAM mode, False to disable.

        Returns:
            dict: The server's response as a dictionary, or None if the request fails.
        """
        response = requests.post(
            f"{self.base_url}/api/lowvramsetting",
    ...
    def display_server_info(self):
        ...
        if self.current_settings:
            capabilities = {
                ...
                "DeepSpeed Enabled": self.current_settings.get(
                ...
                "Generation Speed Capable": self.current_settings.get(
                    "generationspeed_capable", False
                ),
                "Current Generation Speed": self.current_settings.get(
                    "generationspeed_set", "N/A"
                ),
                "Pitch Capable": self.current_settings.get("pitch_capable", False),
                "Current Pitch": self.current_settings.get("pitch_set", "N/A"),
                "Temperature Capable": self.current_settings.get(
                    "temperature_capable", False
                ),
                "Current Temperature": self.current_settings.get(
                    "temperature_set", "N/A"
                ),
                "Streaming Capable": self.current_settings.get(
                    "streaming_capable", False
                ),
                "Multi-voice Capable": self.current_settings.get(
                    "multivoice_capable", False
                ),
                "Multi-model Capable": self.current_settings.get(
                    "multimodel_capable", False
                ),
                "Languages Capable": self.current_settings.get(
